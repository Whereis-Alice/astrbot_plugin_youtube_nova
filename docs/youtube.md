...
下载     Range 下载；DASH 由 ffmpeg 合并；按独立策略压缩
投递     普通视频；超限时可选 OneBot QQ 群文件/私聊文件
```
...
- `ffmpeg`：合并 DASH 音视频、执行可选的视频压缩策略；
...
| 可发送视频体积上限 | 48 MiB | 普通视频消息边界；不控制压缩是否触发 |
| 超限视频投递方式 | 信息与封面 | 可改为 OneBot QQ 群文件/私聊文件 |
| 视频压缩 | 超过 48 MiB | 可关闭、始终执行或改用自定义阈值与编码参数 |

热评开关位于“消息输出 -> 附加内容：热评 -> YouTube”。

yt-dlp 优先选择 DASH，其次是带音频的单文件流，最后才是纯视频轨。格式大小优先读取 `filesize` / `filesize_approx`，缺失时按码率和时长估算。启用压缩或文件投递时，选流预算使用管理员硬上限，以便后处理拿到质量更高的源；否则使用普通视频发送上限。所有候选都超出预算时会选择最小格式，下载后仍会按实际体积复检。
...
- “视频大小上限”限制允许下载的最大文件；
- “可发送视频体积上限”只限制普通视频消息；
- 压缩按独立模式和阈值执行，不再隐式绑定普通发送上限；
- 压缩后的结果仍超限时，可通过 `upload_group_file` 上传到当前 QQ 群。

文件投递支持 aiocqhttp / OneBot v11：群聊调用 `upload_group_file`，私聊调用 `upload_private_file`，只发送到当前会话。NapCat、LLOneBot / LLBot 提供这两个接口，其他协议端按实际能力处理。收到完成回执后才清理缓存；回执未确认时保留缓存至少 10 分钟且不自动重发。接口不支持或明确拒绝时提示原因和原始链接，其他平台按“信息与封面”回退。

压缩默认继续兼容旧行为：总开关开启、模式为“超过指定体积”，阈值和目标体积填 0 时跟随普通发送上限。管理员也可以改为始终压缩，并自定义编码器、preset、分辨率、帧率、视频/音频码率、CRF、重试次数和附加 ffmpeg 参数。附加参数通过参数数组交给 ffmpeg，不经过 shell。
