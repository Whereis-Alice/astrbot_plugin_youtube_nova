...
| 超限视频投递方式 | 信息与封面 | 可改为 OneBot QQ 群文件 |
| 视频压缩 | 超过 48 MiB | 可关闭、始终执行或改用自定义阈值与编码参数 |

热评开关位于“消息输出 -> 附加内容：热评 -> YouTube”。

yt-dlp 优先选择 DASH，其次是带音频的单文件流，最后才是纯视频轨。格式大小优先读取 `filesize` / `filesize_approx`，缺失时按码率和时长估算。启用压缩或群文件投递时，选流预算使用管理员硬上限，以便后处理拿到质量更高的源；否则使用普通视频发送上限。所有候选都超出预算时会选择最小格式，下载后仍会按实际体积复检。
...
群文件只支持 aiocqhttp / OneBot v11 的 QQ 群聊。插件兼容 NapCat、LLOneBot / LLBot 与 SnowLuma 的标准 action；文件上传完成前会保留本地缓存，action 成功返回后才清理。私聊和其他平台按“信息与封面”回退。
